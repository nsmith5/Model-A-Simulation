...
public class PlotArea : DrawingArea
{
    ...
    private Gdk.Colorspace colorspace;     // Colorspace of pixel buffer

    public PlotArea (int width, int height)
    {
        colorspace = Gdk.Colorspace.RGB;            // RGB Pixels
        uint8[] data = new uint8[3*width*height];   // Pixel pixel array
        for (int i = 0; i<3*width*height; i++)
        {
            data[i] = 100;
        }
        buffer = new Gdk.Pixbuf.from_data (data, colorspace, false, 8, width, height, 3*width);
    ...
    public void update_data (double[] field)
    {
        ...
        {
            pixels[3*i + 0] = (uint8)((field[i]+ 5)*255.0/10.0);
            pixels[3*i + 1] = (uint8)((field[i]+ 5)*255.0/10.0);
            pixels[3*i + 2] = (uint8)((field[i]+ 5)*255.0/10.0);
        }
        return;
    }
}
